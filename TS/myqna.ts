...
const fetchQuestions = async () => {
    ...
    try {
        const api:string = `http://localhost:4900/myques-data/${row}/${limit}`;
...
const renderQues = (questionsToDisplay:any) => {
    questionsToDisplay.forEach((quest:any) => {
        ...
        `;
        if(quesList) quesList.appendChild(questionCard); // Append the newly created card
...
// Open Answers box
const openShowAnsBox = async (id:string, ques:any) => {
    ...
    try {
        ...
        if(answersContainer) answersContainer.innerHTML = '';  // Clear any previous content
...
//render answers
const renderAnswers = (data1:any, id:string, ques:any, data2:any) => {
    const answersContainer = document.getElementById(`answers-container-${id}`);
...
async function deleteQuestion(uuid:string) {
    try {
       // console.log(uuid);
...
function showDeleteConfirmation(uuid:string) {
    ...
    const message:HTMLDivElement = document.createElement("p");
